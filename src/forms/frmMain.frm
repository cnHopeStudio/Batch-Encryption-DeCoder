...
Private Function ReadInFile()
    ...
    ReDim Preserve bytBinTemp(0)

    Open strInFile For Binary Access Read As #intFileNum
    Do Until EOF(intFileNum)
        DoEvents
        Get #intFileNum, , bytBinTemp(lngIndex)
        lngIndex = lngIndex + 1
        ReDim Preserve bytBinTemp(lngIndex)
    Loop
    Close #intFileNum
    
    If bytBinTemp(0) = &HFF And bytBinTemp(1) = &HFE Then
...
Public Function DeCodeAndOutputEx()
    Dim strEncryptionHeader As String
    Dim strHeader As String
    Dim strSourceCode As String ' 源码
...
' 初始化文件头
Private Function InitHeader(strSrc As String) As String
    Dim strTemp
    
    strTemp = strSrc
    
    ' "%" 在下一行
    Do While InStr(strTemp, vbCrLf) < InStr(strTemp, "%")
        strTemp = Mid(strTemp, InStr(strTemp, vbCrLf) + 2)
    Loop
